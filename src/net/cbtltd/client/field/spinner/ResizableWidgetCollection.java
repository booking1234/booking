...
/**
 * The Class ResizableWidgetCollection is a collection of {@link ResizableWidget} that periodically 
 * checks the outer dimensions of a widget and redraws it as necessary. Every
 * {@link ResizableWidgetCollection} uses a timer, so consider the cost when adding one.
 * 
...
implements WindowResizeListener, Iterable<ResizableWidget> {
	...
	private Timer resizeCheckTimer = new Timer() {
		...
		public void run() {
			...
			} catch(RuntimeException x){Log.debug("ResizableWidgetCollection windowHeight " + windowHeight + " windowWidth " + windowWidth);}
		}
	};

...
import net.cbtltd.client.field.datepicker.DefaultCalendarView.CellGrid.DateCell;
	...
	class CellGrid extends CellGridImpl<java.util.Date> {
		...
		class DateCell extends Cell {
			...
			DateCell(Element td, boolean isWeekend) {
				super(td, new Date());
				cellStyle = AbstractField.CSS.cbtDatePickerDay();
				if (isWeekend) {cellStyle += " " + AbstractField.CSS.cbtDatePickerDayIsWeekend();}
			}
			...
			public void update(Date current) {
				...
				setText(value);
				dateStyle = cellStyle;
				if (isFiller()) {dateStyle += " " + AbstractField.CSS.cbtDatePickerDayIsFiller();} 
				else {
			...
			public void updateStyle() {
				...
				if (isHighlighted()) {
					accum += " " + AbstractField.CSS.cbtDatePickerDayIsValueAndHighlighted();
					if (isHighlighted() && isSelected()) {accum += " " + AbstractField.CSS.cbtDatePickerDayIsValueAndHighlighted();}
				}
				if (!isEnabled()) {accum += " " + AbstractField.CSS.cbtDatePickerDayIsDisabled();}
				setStyleName(accum);
			}
	...
	public void setup() {
		...
		for (int i = 0; i < CalendarModel.DAYS_IN_WEEK; i++) {
			...
			if (CalendarUtil.isWeekend(dayIdx)) {
				formatter.setStyleName(0, i, AbstractField.CSS.cbtDatePickerWeekendLabel());
				...
				formatter.setStyleName(0, i, AbstractField.CSS.cbtDatePickerWeekdayLabel());
			}
		}
		...
		initWidget(grid);
		grid.setStyleName(AbstractField.CSS.cbtDatePickerDays());
	}

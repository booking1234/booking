...
public final class _Test implements Handleable {
	...
	public static void main(String args[]) throws Exception {
		...
		LOG.error("WebChalet Test Started");
		SqlSession sqlSession = RazorServer.openSession();
		
		try {
			
			String altpartyid = "179897"; //WebChalet party ID
			//String reservationid = "7233443"; //WebChalet reservation

			Partner partner = sqlSession.getMapper(PartnerMapper.class).exists(sqlSession.getMapper(PartnerMapper.class).partyIDFromEmail(HasUrls.PARTY_WEBCHALET_EMAIL));
...
//			handler.cancelReservation(sqlSession, reservation);
		...
		LOG.error("WebChalet Terminated not an error. ");
		System.exit(0);
	}

	private A_Handler getHandler() {
		SqlSession sqlSession = RazorServer.openSession();
		String altpartyid = "325643"; //UAT server

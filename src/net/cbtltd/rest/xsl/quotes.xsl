...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/quotes/quote">
    <html>
      <head>
        ...
        <style type="text/css">
          ...
          table th, table td { text-align: left; padding: 2px; }
          ...
          table, tr, th, td {border-collapse: collapse; margin: 0; border: 0 solid gray;}
          .fr {float: right; padding: 2px; margin-right: 10px;}
        </style>
      </head>
      <body>
        <xsl:if test="not(message)">
        <table>
          <thead><tr><th colspan="2"><h3><xsl:value-of select="productname" /><div class="fr"><xsl:value-of select="currency" />&#160;<xsl:value-of select="quote" /></div></h3></th></tr></thead>
          <tbody>

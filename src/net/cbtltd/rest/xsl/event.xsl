...
<xsl:stylesheet version="1.0"
  ...
  <!--
    Styling for journal events.
  -->
  <xsl:template match="/events">
    <html>
      <head>
        <title>Journal Events</title>
        <style type="text/css">
          ...
          div, table td { background-color: #fff; border-top: 0 solid #ddd; vertical-align: top;}
          table, tr, th, td {border-collapse: collapse; margin: 0; border: 0 solid gray; padding: 1px 4px;}
          .event {border-bottom: 10px solid #ddd;} 
        ...
        <table>
          ...
          <tbody>
            <xsl:for-each select="event">
	      	...
	      	<td colspan="8"><xsl:value-of select="eventname" /></td>
  ...
  <xsl:template match="event/journal">
      <tr>
        <td colspan="3"> </td>
